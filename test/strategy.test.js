...
describe('Core Strategy', () => {
  describe('Initialisation', () => {
    ...
    it('should reject minValue less than zero', () => {
      assert.throws(() => {
        new Strategy({minValue: -1})
      }, /minValue must be at least 0/);
    });

    it('should reject maxValue less than 1', () => {
      assert.throws(() => {
        new Strategy({maxValue: 0})
      }, /maxValue must be at least 1/);
    });

    it('should reject maxValue less than minValue', () => {
      assert.throws(() => {
        new Strategy({maxValue: 50, minValue: 51})
      }, /minValue must be less than maxValue/);
    });

    it('should reject i less than zero', () => {
      assert.throws(() => {
        new Strategy({i: -1})
    ...
    it('should initialise everything when passed', () => {
      let strat = new Strategy({minValue: 100, maxValue: 500, i: 5, multiplier: 200, randomisationFactor: 0.5, zeroMeansZero: false})
  ...
  describe('clampAndRandomise(i, value)', () => {
    it('should return 0 when zeroMeansZero is true and i=0', () => {
      let strat = new Strategy({zeroMeansZero: true});
      assert.equal(strat.clampAndRandomise(0, 100), 0);
    });

    it('should return *value* when zeroMeansZero is false and i=0', () => {
      let strat = new Strategy({zeroMeansZero: false});
      assert.equal(strat.clampAndRandomise(0, 100), 100);
    });

    it('should return *value* when i=2', () => {
      let strat = new Strategy();
      assert.equal(strat.clampAndRandomise(2, 100), 100);
    });

    it('should return 500 when minValue=500 and value=100', () => {
      let strat = new Strategy({minValue: 500});
      assert.equal(strat.clampAndRandomise(5, 100), 500);
    });

    it('should return 500 when maxValue=500 and value=9999', () => {
      let strat = new Strategy({maxValue: 500});
      assert.equal(strat.clampAndRandomise(5, 9999), 500);
    });

    it('should return 50 when multipler=10 and i=5', () => {
      let strat = new Strategy({multiplier: 10});
      assert.equal(strat.clampAndRandomise(5, 5), 50);
    });

    it('should return randomly between >=50 and <= 100 when multipler=10 and i=5 and randomisationFactor=1', () => {
      let strat = new Strategy({randomisationFactor: 1, multiplier: 10});
      let values = {}, value, i;
      for (i = 0; i < 100; i ++) {
    ...
    it('should return randomly between >=50 and <=75 when multipler=10 and i=5 and randomisationFactor=0.5', () => {
      let strat = new Strategy({randomisationFactor: 0.5, multiplier: 10});
  ...
  describe('reset()', () => {
    it('should set i to 0', () => {
      let strat = new Strategy({i :5});
      assert.equal(strat.i, 5);
      strat.reset();
      assert.equal(strat.i, 0);     
    })
